#pragma once
#include "../MAIN/GAME_BASE.h"
#include<time.h>
#include<conio.h>
#include<ctype.h>
#include<string>
#include<stdio.h>
#include<iostream>
#include<chrono>
#include<thread>
#include<random>
#include<map>
#include<cstring>
#include"../../libOne/inc/input.h"
#define Qnumber 100

namespace GAME12
{
    class GAME :
        public GAME_BASE
    {
    public:
        GAME(class MAIN* main) :GAME_BASE(main) {};
        ~GAME() {};
        int create();
        void proc();
        void destroy();
    private:
        enum STATE { TITLE, INIT, WAIT, PLAY1, PLAY2, PLAY3, PLAY4, PLAY5, OVER1, OVER2, OVER3, OVER4, OVER5, END };
        STATE State = TITLE;
        void title();
        void init();
        void wait();
        void play1();
        void play2();
        void play3();
        void play4();
        void play5();
        void over1();
        void over2();
        void over3();
        void over4();
        void over5();
        void end();
        void recordInputToKana();
        void draw1(int snum);
        void draw2(int snum);
        void draw3(int snum);
        void draw4(int snum);
        void draw5(int snum);
        void randomnum();
        INPUT_CODE gettriggerkeycode();

        int num[Qnumber] = {}; 
        int rnum, number, flag, set, count, stop;
        int background = 0;
        int teachimage = 0;
        int pretwoimage = 0;
        int twoimage = 0;
        int preoneimage = 0;
        int oneimage = 0;
        int back1 = 0;
        int back2 = 0;
        int back3 = 0;
        int back4 = 0;
        int back5 = 0;
        int missimage = 0;
        int overback = 0;
        int overmale = 0;
        int overfemale = 0;
        int clearback = 0;
        int clearmale = 0;
        int clearfemale = 0;
        int backbgm = 0;
        int playbgm = 0;
        int correctsound = 0;
        int incorrectsound = 0;
        int oversound = 0;
        int endsound = 0;

        std::string ch;
        float curtime, mflag;
        std::string currentInput; // Œ»Ý‚Ì“ü—Í•¶Žš—ñ

        const char* pretwoWord[Qnumber][2] = {
            {"’š”J","‚Ä‚¢‚Ë‚¢"},
            {"ŠjS","‚©‚­‚µ‚ñ"},
            {"“Š","‚Æ‚¤‚©‚Â"},
            {"–Œi","‚¶‚å‚¯‚¢"},
            {"“Â‘£","‚Æ‚­‚»‚­"},
            {"–Ó]","‚à‚¤‚¶‚ã‚¤"},
            {"–Ô—…","‚à‚¤‚ç"},
            {"ŠÕŽU","‚©‚ñ‚³‚ñ"},
            {"–ï‰î","‚â‚Á‚©‚¢"},
            {"À‘T","‚´‚º‚ñ"},
            {"‹Å","‚ ‚©‚Â‚«"},
            {"“«‹M","‚Æ‚¤‚«"},
            {"‹á–¡","‚¬‚ñ‚Ý"},
            {"’I“c","‚½‚È‚¾"},
            {"–î–Ê","‚â‚¨‚à‚Ä"},
            {"—ÖŠs","‚è‚ñ‚©‚­"},
            {"F","‚¶‚ã‚ñ‚µ‚å‚­"},
            {"‰º—Ÿ","‚°‚è"},
            {"’·ŽÖ","‚¿‚å‚¤‚¾"},
            {"—]‰C","‚æ‚¢‚ñ"},
            {"ŽÀ‘H","‚¶‚Á‚¹‚ñ"},
            {"ŒŒŠá","‚¿‚Ü‚È‚±"},
            {"‘S”p","‚º‚ñ‚Ï‚¢"},
            {"”áy","‚Ð‚¶‚ã‚ñ"},
            {"f•ƒ","‚¨‚¶"},
            {"‘ƒ","‚»‚¤‚·‚¢"},
            {"ˆŸ—¬","‚ ‚è‚ã‚¤"},
            {"–¼Žc","‚È‚²‚è"},
            {"‘Ê•¶","‚¾‚Ô‚ñ"},
            {"–ƒŒ","‚Ü‚·‚¢"},
            {"‹Þ’æ","‚«‚ñ‚Ä‚¢"},
            {"a","‚Ý‚¼"},
            {"•Î‹ü","‚Ö‚ñ‚­‚Â"},
            {"X‘Ô","‚µ‚ã‚¤‚½‚¢"},
            {"”h”´","‚Í‚Î‚Â"},
            {"—¾•ê","‚è‚å‚¤‚Ú"},
            {"“‹Ú","‚Æ‚¤‚³‚¢"},
            {"’èŠ¼","‚Ä‚¢‚©‚ñ"},
            {"Š_ª","‚©‚«‚Ë"},
            {"‹”N","‚«‚å‚¤‚Ë‚ñ"},
            {"•â²","‚Ù‚³"},
            {"ˆŸ‰”","‚ ‚¦‚ñ"},
            {"§ˆÓ","‚±‚ñ‚¢"},
            {"r‘«","‚µ‚ã‚ñ‚»‚­"},
            {"ÞŽæ","‚¹‚Á‚µ‚ã"},
            {"‘å˜g","‚¨‚¨‚í‚­"},
            {"Ð‰Ð","‚³‚¢‚©"},
            {"´–{","‚µ‚å‚¤‚Ù‚ñ"},
            {"‡–‚","‚·‚¢‚Ü"},
            {"–J”ü","‚Ù‚¤‚Ñ"},
            {"–~Í","‚Ú‚ñ‚³‚¢"},
            {"–•Á","‚Ü‚Á‚µ‚å‚¤"},
            {"–ù‰x","‚ä‚¦‚Â"},
            {"v‘¬","‚¶‚ñ‚»‚­"},
            {"¼‹\","‚³‚¬"},
            {"Ž¦´","‚µ‚³"},
            {"•ñV","‚Ù‚¤‚µ‚ã‚¤"},
            {"‰Ðª","‚©‚±‚ñ"},
            {"Šæä","‚ª‚ñ‚¶‚å‚¤"},
            {"”}‰î","‚Î‚¢‚©‚¢"},
            {"‹‡”—","‚«‚ã‚¤‚Í‚­"},
            {"‰Ç­","‚©‚µ‚å‚¤"},
            {"•¹“Ç","‚Ö‚¢‚Ç‚­"},
            {"ˆÀˆí","‚ ‚ñ‚¢‚Â"},
            {"IÙ","‚±‚¤‚¹‚Â"},
            {"ŠÒ—ï","‚©‚ñ‚ê‚«"},
            {"Œ£g","‚¯‚ñ‚µ‚ñ"},
            {"‘{õ","‚»‚¤‚³‚­"},
            {"‘Š–o","‚·‚à‚¤"},
            {"ù•—","‚¹‚ñ‚Õ‚¤"},
            {"•¾ŽÐ","‚Ö‚¢‚µ‚á"},
            {"‹ì’€","‚­‚¿‚­"},
            {"”eŽÒ","‚Í‚µ‚á"},
            {"—…—ñ","‚ç‚ê‚Â"},
            {"‹ó‘a","‚­‚¤‚»"},
            {"“Ž—","‚±‚­‚¶"},
            {"Ž©l","‚¶‚µ‚ã‚­"},
            {"Š±Â","‚©‚ñ‚µ‚å‚¤"},
            {"‘sŒ’","‚»‚¤‚¯‚ñ"},
            {"Žèj","‚½‚Ã‚È"},
            {"Š„ˆ¤","‚©‚Â‚ ‚¢"},
            {"Ž©–“`","‚¶‚¶‚å‚Å‚ñ"},
            {"Š´–Á","‚©‚ñ‚ß‚¢"},
            {"lŠ_","‚Ð‚Æ‚ª‚«"},
            {"’¹”§","‚Æ‚è‚Í‚¾"},
            {"“O’ê","‚Ä‚Á‚Ä‚¢"},
            {"‹@Œ™","‚«‚°‚ñ"},
            {"r—Á","‚±‚¤‚è‚å‚¤"},
            {"‹êa","‚­‚¶‚ã‚¤"},
            {"ŽŒÈS","‚±‚Á‚«‚µ‚ñ"},
            {"’§í","‚¿‚å‚¤‚¹‚ñ"},
            {"–ì•é","‚â‚Ú"},
            {"„’¼","‚²‚¤‚¿‚å‚­"},
            {"ŠL’Ë","‚©‚¢‚Ã‚©"},
            {"ŠÕE","‚©‚ñ‚µ‚å‚­"},
            {"O","‚­‚¿‚Ñ‚é"},
            {"‰ð–U","‚©‚¢‚Ú‚¤"},
            {"X“R","‚±‚¤‚Ä‚Â"},
            {"’­–]","‚¿‚å‚¤‚Ú‚¤"},
            {"Šg[","‚©‚­‚¶‚ã‚¤"},
        };

        const char* twoWord[Qnumber][2] = {
            {"—”_","‚ç‚­‚Ì‚¤"},
            {"œb•ñ","‚¢‚Ù‚¤"},
            {"™‹“ß","‚¹‚Â‚È"},
            {"r‘å","‚¶‚ñ‚¾‚¢"},
            {"“Mˆ¤","‚Å‚«‚ ‚¢"},
            {"•p”­","‚Ð‚ñ‚Ï‚Â"},
            {"®‘","‚µ‚å‚¤‚»‚¤"},
            {"m’è","‚±‚¤‚Ä‚¢"},
            {"áŒk","‚¹‚Á‚¯‚¢"},
            {"’„Žq","‚¿‚á‚­‚µ"},
            {"‰°–Ê","‚¨‚­‚ß‚ñ"},
            {"‘šŠQ","‚»‚¤‚ª‚¢"},
            {"ˆêR","‚¢‚Á‚µ‚ã‚¤"},
            {"“yë","‚Ç‚¶‚å‚¤"},
            {"ù‰ñ","‚¹‚ñ‚©‚¢"},
            {"Xˆ«","‚µ‚ã‚¤‚ ‚­"},
            {"˜I’æ","‚ë‚Ä‚¢"},
            {"Œ°’˜","‚¯‚ñ‚¿‚å"},
            {"‘Ž£","‚±‚­‚¶"},
            {"‰ïŽß","‚¦‚µ‚á‚­"},
            {"“y’dê","‚Ç‚½‚ñ‚Î"},
            {"D’Qê","‚µ‚ã‚¤‚½‚ñ‚Î"},
            {"‰ñŒü","‚¦‚±‚¤"},
            {"”ë–Æ","‚Ð‚ß‚ñ"},
            {"”Ð•z","‚Í‚ñ‚Õ"},
            {"‹ºŠd","‚«‚å‚¤‚©‚­"},
            {"‰á’Œ","‚©‚Î‚µ‚ç"},
            {"ÓÎ","‚³‚¢‚¹‚«"},
            {"´–ó","‚µ‚å‚¤‚â‚­"},
            {"žB–†","‚ ‚¢‚Ü‚¢"},
            {"Ží•c","‚µ‚ã‚Ñ‚å‚¤"},
            {"ˆÞk","‚¢‚µ‚ã‚­"},
            {"Žvõ","‚µ‚³‚­"},
            {"Ž½","‚¤‚é‚µ"},
            {"»ŒC","‚¹‚¢‚©"},
            {"ˆôA","‚¢‚ñ‚±‚¤"},
            {"“¶‰Ì","‚í‚ç‚×‚¤‚½"},
            {"‰Ò“­","‚©‚Ç‚¤"},
            {"’üŒ¸","‚Ä‚¢‚°‚ñ"},
            {"ˆÖŽq","‚¢‚·"},
            {"¾–ñ‘","‚¹‚¢‚â‚­‚µ‚å"},
            {"“\—p","‚¿‚å‚¤‚æ‚¤"},
            {"Ž†•¼","‚µ‚Ö‚¢"},
            {"Šëœœ","‚«‚®"},
            {"‰…¦","‚¦‚ñ‚±‚ñ"},
            {"ˆ¥ŽA","‚ ‚¢‚³‚Â"},
            {"ˆ¶æ","‚ ‚Ä‚³‚«"},
            {"ŽY’…","‚¤‚Ô‚¬"},
            {"‘a‰“","‚»‚¦‚ñ"},
            {"“MŽ€","‚Å‚«‚µ"},
            {"ˆØ•|","‚¢‚Ó"},
            {"‹¶–z","‚«‚å‚¤‚Ù‚ñ"},
            {"ˆ¤”L","‚ ‚¢‚Ñ‚å‚¤"},
            {"ŸT‹ü","‚¤‚Á‚­‚Â"},
            {"ù—¥","‚¹‚ñ‚è‚Â"},
            {"—d‰","‚æ‚¤‚¦‚ñ"},
            {"‰ ·","‚¨‚¤‚¹‚¢"},
            {"ç…˜r","‚ç‚Â‚í‚ñ"},
            {"‰ß“","‚©‚±‚­"},
            {"˜e• ","‚í‚«‚Î‚ç"},
            {"’úŠÏ","‚Ä‚¢‚©‚ñ"},
            {"i’»","‚µ‚ñ‚¿‚å‚­"},
            {"ž","‚·‚»"},
            {"¦ŽS","‚¹‚¢‚³‚ñ"},
            {"–¨ŒŽ","‚Ý‚Â‚°‚Â"},
            {"eÊ","‚µ‚ñ‚¹‚«"},
            {"Ž¸çH","‚µ‚Á‚»‚¤"},
            {"•ß‘¨","‚Ù‚»‚­"},
            {"‘ÁŠü","‚¾‚«"},
            {"“µE","‚Ç‚¤‚±‚¤"},
            {"”Ä—p«","‚Í‚ñ‚æ‚¤‚¹‚¢"},
            {"–u”­","‚Ú‚Á‚Ï‚Â"},
            {"“©–è","‚Æ‚¤‚â"},
            {"‰°’f","‚¨‚­‚¾‚ñ"},
            {"f’v","‚ç‚¿"},
            {"‰¡•¿","‚¨‚¤‚Ö‚¢"},
            {"Š¢‰ð","‚ª‚©‚¢"},
            {"o—•","‚µ‚ã‚Â‚ç‚ñ"},
            {"ŠR","‚ª‚¯"},
            {"–¾—Ä","‚ß‚¢‚è‚å‚¤"},
            {"Šß–¡","‚ª‚ñ‚Ý"},
            {"˜d˜G","‚í‚¢‚ë"},
            {"‹ð˜M","‚®‚ë‚¤"},
            {"‹Í·","‚«‚ñ‚³"},
            {"âÄé","‚ë‚¤‚¶‚å‚¤"},
            {"ŽR˜[","‚³‚ñ‚ë‚­"},
            {"Œ®”Õ","‚¯‚ñ‚Î‚ñ"},
            {"‰…”O","‚¨‚ñ‚Ë‚ñ"},
            {"Š{","‚ ‚²"},
            {"ŒÕŒû","‚±‚±‚¤"},
            {"‹T—ô","‚«‚ê‚Â"},
            {"’E‰P","‚¾‚Á‚«‚ã‚¤"},
            {"škŠo","‚«‚ã‚¤‚©‚­"},
            {"^•","‚µ‚ñ‚µ"},
            {"‰aH","‚¦‚¶‚«"},
            {"Ž¶Ó","‚µ‚Á‚¹‚«"},
            {"‘A–]","‚¹‚ñ‚Ú‚¤"},
            {"»—’","‚·‚È‚ ‚ç‚µ"},
            {"ˆØŒh","‚¢‚¯‚¢"},
            {"Œêœb","‚²‚¢"},
        };

        const char* preoneWord[Qnumber][2] = {
            {"“æˆó","‚È‚Â‚¢‚ñ"},
            {"‰Žp","‚ ‚Å‚·‚ª‚½"},
            {"•ã•J","‚Ù‚Ð‚Â"},
            {"Ž¹“i","‚µ‚Á‚Æ"},
            {"–Ø‘ö","‚Ú‚­‚½‚­"},
            {"”ªd—¨","‚â‚¦‚Þ‚®‚ç"},
            {"ˆÇ—Ñ","‚«‚å‚¤‚è‚ñ"},
            {"‰XŽÉ","‚«‚ã‚¤‚µ‚á"},
            {"•L¶","‚Ð‚Á‚¹‚¢"},
            {"“t‹à","‚ß‚Á‚«"},
            {"“C’k","‚Ä‚¢‚¾‚ñ"},
            {"žwî","‚Ð‚¨‚¤‚¬"},
            {"‹mº","‚¿‚ñ‚¹‚¢"},
            {"‘ÍÏ","‚½‚¢‚¹‚«"},
            {"–é³","‚â‚µ‚á"},
            {"‹§³","‚«‚å‚¤‚¹‚¢"},
            {"O¾","‚®‚º‚¢"},
            {"‘­Œ¿","‚¼‚­‚°‚ñ"},
            {"•“Ž«","‚Ô‚¶"},
            {"^”n","‚¶‚ã‚¤‚Î"},
            {"–î’L","‚â‚½‚ç"},
            {"—K÷","‚ä‚¤‚¶‚å‚¤"},
            {"Œ`Š[","‚¯‚¢‚ª‚¢"},
            {"“´Ž@","‚Ç‚¤‚³‚Â"},
            {"oŠH","‚¶‚ñ‚©‚¢"},
            {"•«‡","‚Ó‚ñ‚²‚¤"},
            {"ˆê•Ë","‚¢‚¿‚×‚Â"},
            {"Š•‹Ð","‚Æ‚«‚ñ"},
            {"ˆò‰Â","‚¢‚ñ‚©"},
            {"ˆ ‘R","‚ ‚º‚ñ"},
            {"WS","‚µ‚ã‚¤‚µ‚ã‚¤"},
            {"–Œü","‚·‚¤‚±‚¤"},
            {"•_‹c","‚Ñ‚å‚¤‚¬"},
            {"—†ù","‚ç‚¹‚ñ"},
            {"ŽÜ”M","‚µ‚á‚­‚Ë‚Â"},
            {"’ÏŽÏ","‚Â‚­‚¾‚É"},
            {"šâ’†","‚±‚¿‚ã‚¤"},
            {"—½‰_","‚è‚å‚¤‚¤‚ñ"},
            {"Žðæ","‚µ‚ã‚±‚¤"},
            {"–F†","‚Ù‚¤‚¶‚ã‚ñ"},
            {"˜V–ê","‚ë‚¤‚â"},
            {"ë’[","‚¹‚ñ‚½‚ñ"},
            {"Ž“ŽÇ","‚ß‚µ‚×"},
            {"’X‘R","‚½‚ñ‚º‚ñ"},
            {"’¨’B","‚¿‚å‚¤‚½‚Â"},
            {"”X’†","‚Ì‚¤‚¿‚ã‚¤"},
            {"“é’v","‚¶‚ã‚ñ‚¿"},
            {"’\y","‚½‚ñ‚·"},
            {"“q”Ž","‚Æ‚Î‚­"},
            {"–“ª","‚¿‚ñ‚Æ‚¤"},
            {"éò‘J","‚¨‚¤‚¹‚ñ"},
            {"‹¢n","‚«‚å‚¤‚¶‚ñ"},
            {"•ÂÇ","‚Ö‚¢‚»‚­"},
            {"‰Ç","‚¦‚ñ‚»‚­"},
            {"¦â","‚¹‚¢‚º‚Â"},
            {"ŠË–n","‚©‚ñ‚Ú‚­"},
            {"•â“U","‚Ù‚Ä‚ñ"},
            {"Šˆà","‚¢‚í‚ä‚é"},
            {"”ÝŒì","‚Ð‚²"},
            {"ŽÄ”à","‚³‚¢‚Ð"},
            {"‘›ï","‚»‚¤‚¶‚å‚¤"},
            {"•‚Žî","‚Ó‚µ‚ã"},
            {"ÆŽã","‚º‚¢‚¶‚á‚­"},
            {"‹f–â","‚«‚­‚à‚ñ"},
            {"Žôp","‚¶‚ã‚¶‚ã‚Â"},
            {"•M–N","‚Ð‚Á‚Û‚¤"},
            {"Œª‘»","‚¯‚ñ‚»‚ñ"},
            {"Ò—˜","‚³‚¢‚è"},
            {"³‰[","‚¹‚¢‚¶‚ã‚ñ"},
            {"x«","‚¶‚ñ‚¹‚¢"},
            {"ŽWŽW","‚³‚ñ‚³‚ñ"},
            {"Šõˆø","‚«‚Ñ‚«"},
            {"‰–è","‚¦‚ñ‚â"},
            {"ŒÏ’K","‚±‚è"},
            {"—¥˜C","‚è‚Â‚è‚å"},
            {"‰¦","‚Ó‚·‚Ü"},
            {"’—–ƒ","‚¿‚å‚Ü"},
            {"”Ò‰Ì","‚Î‚ñ‚©"},
            {"Œrü","‚¯‚¢‚¹‚ñ"},
            {"˜c‹È","‚í‚¢‚«‚å‚­"},
            {"–SŠ[","‚È‚«‚ª‚ç"},
            {"‰²åy","‚©‚«"},
            {"ŠW¢","‚ª‚¢‚¹‚¢"},
            {"–žo","‚Ü‚ñ‚±‚¤"},
            {"’FˆÊ","‚»‚ñ‚¢"},
            {"–ãâ","‚à‚ñ‚º‚Â"},
            {"’ÖŽõ","‚¿‚ñ‚¶‚ã"},
            {"–Þ•¨","‚ä‚¤‚Ô‚Â"},
            {"‰dŽ™","‚¦‚¢‚¶"},
            {"”Z‰","‚Ì‚¤‚¦‚ñ"},
            {"ˆßŒ…","‚¢‚±‚¤"},
            {"‹Ô•ç","‚«‚ñ‚Ú"},
            {"ŠWŠ»","‚ª‚¢‚©‚ñ"},
            {"‰X”ì","‚«‚ã‚¤‚Ð"},
            {"“ s","‚Æ‚¤‚¶‚ñ"},
            {"ŠO‰™","‚ª‚¢‚¹‚¢"},
            {"•Æ‰“","‚Ö‚«‚¦‚ñ"},
            {"–ìŒÏ","‚â‚±"},
            {"ŒŒÅ","‚°‚ñ‚±"},
            {"Œ–“`","‚¯‚ñ‚Å‚ñ"},

        };
        const char* oneWord[Qnumber][2] = {
            {"àñ—í","‚©‚¢‚ê‚¢"},
            {"‰GêF","‚¤‚¶‚á‚­"},
            {"‰ð—b","‚©‚¢‚æ‚¤"},
            {"ˆÀ˜Ã","‚ ‚ñ‚¢‚Â"},
            {"ŒÓ—","‚¤‚ë‚ñ"},
            {"ç´‘²","‚ç‚»‚Â"},
            {"èŠ‘R","‚°‚«‚º‚ñ"},
            {"ŠÔŒ„","‚©‚ñ‚°‚«"},
            {"ç˜¾","‚¹‚ñ‚¶‚ñ"},
            {"NW","‚µ‚ã‚¤‚µ‚ã‚¤"},
            {"š@ŽÚ","‚µ‚¹‚«"},
            {"Š Šn","‚ª‚¢‚©‚­"},
            {"ó”K","‚ ‚³‚¬"},
            {"‹wQ","‚«‚ã‚¤‚µ‚ã‚¤"},
            {"à‰‘R","‚¯‚¢‚º‚ñ"},
            {"”òâI","‚Â‚Ô‚Ä"},
            {"ŠÖè±","‚©‚ñ‚µ‚å"},
            {"ŒÚá¾","‚±‚×‚ñ"},
            {"äe—ñ","‚ë‚ê‚Â"},
            {"ŠíŽM","‚«‚×‚¢"},
            {"ˆ³ó","‚¨‚¤‚¶‚å‚¤"},
            {"ˆê‹d","‚¢‚Á‚«‚­"},
            {"•§êœ","‚Ô‚Â‚ª‚ñ"},
            {"’o’£","‚µ‚¿‚å‚¤"},
            {"á·Ÿù","‚©‚ñ‚»‚¤"},
            {"˜Ã—V","‚¢‚Â‚ä‚¤"},
            {"›c‘","‚Ç‚ñ‚»‚¤"},
            {"æ‡Œ¾","‚ä‚°‚ñ"},
            {"š’Žq","‚Í‚â‚µ"},
            {"‰ù›s","‚©‚¢‚æ‚¤"},
            {"–¾âH","‚Ý‚å‚¤‚Î‚ñ"},
            {"•úžD","‚Ù‚¤‚±‚¤"},
            {"_éa","‚µ‚ñ‚¹‚ñ"},
            {"‹°œô","‚«‚å‚¤‚­"},
            {"âÑ“H","‚¦‚ñ‚Ä‚«"},
            {"èÍèÊ","‚ ‚¢‚½‚¢"},
            {"ŠÆžü","‚©‚ñ‚µ‚å‚¤"},
            {"ŒßéM","‚²‚µ‚å‚¤"},
            {"SŒq","‚±‚¤‚¯‚¢"},
            {"å]åS","‚¤‚ñ‚µ‚á"},
            {"à‰à‰","‚¯‚¢‚¯‚¢"},
            {"™p™p","‚×‚«‚×‚«"},
            {"ãÀ’¹","‚µ‚¿‚å‚¤"},
            {"åÔ•Ê","‚×‚¢‚×‚Â"},
            {"Mžµ","‚µ‚ã‚¤‚µ‚ã‚¤"},
            {"ã|F","‚Í‚È‚¾‚¢‚ë"},
            {"ŒŸæI","‚¯‚ñ‚©‚­"},
            {"Ê‘’","‚ê‚ñ‚»‚¤"},
            {"çaç€","‚ ‚Â‚ê‚«"},
            {"Œ¬çh","‚¯‚ñ‚¿"},
            {"èóŽg","‚¢‚µ"},
            {"”H‹X","‚Ë‚¬"},
            {"—¬á¾","‚è‚ã‚¤‚×‚ñ"},
            {"“Ù‘–","‚Æ‚ñ‚»‚¤"},
            {"Š¢âI","‚ª‚ê‚«"},
            {"‰_é¤","‚¤‚ñ‚Ñ‚ñ"},
            {"Š«à","‚©‚ñ‚µ‚å‚¤"},
            {"‹‡D","‚«‚ã‚¤‚µ‚ã‚¤"},
            {"“Ûšƒ","‚Ç‚ñ‚º‚¢"},
            {"êX’¹","‚µ‚¿‚å‚¤"},
            {"—÷É","‚ê‚ñ‚¹‚«"},
            {"ážás","‚æ‚¤‚»"},
            {"Œ—“…","‚¯‚ñ‚Æ‚¤"},
            {"àE‘R","‚³‚ñ‚º‚ñ"},
            {"s•Ê","‚µ‚ã‚ñ‚×‚Â"},
            {"àÂê{","‚±‚¤‚©‚Â"},
            {"Ÿ¼—{","‚©‚ñ‚æ‚¤"},
            {"äwäƒ","‚¶‚­‚ë"},
            {"ŸWŸ`","‚³‚«‚å"},
            {"æÈæ“","‚Ö‚ñ‚½‚­"},
            {"mãZ","‚±‚¤‚¯‚¢"},
            {"æPæO","‚«‚ä"},
            {"‰r‰ù","‚¦‚¢‚©‚¢"},
            {"ošº","‚¶‚ñ‚ ‚¢"},
            {"‹~œ•","‚«‚ã‚¤‚¶‚ã‚Â"},
            {"‘©ãù","‚»‚­‚µ‚ã‚¤"},
            {"’ßšY","‚©‚­‚ê‚¢"},
            {"qŠi","‚©‚ñ‚©‚­"},
            {"áJ•Ê","‚¯‚ñ‚×‚Â"},
            {"é‚–¼","‚¬‚å‚¤‚ß‚¢"},
            {"—ü‘z","‚ê‚ñ‚»‚¤"},
            {"åÃ“Å","‚Æ‚Ç‚­"},
            {"Ÿó’","‚©‚ñ‚¿‚ã‚¤"},
            {"å‰ŒË","‚½‚ñ‚±"},
            {"›Î‘R","‚´‚ñ‚º‚ñ"},
            {"êZår","‚¢‚Â‚Ú‚¤"},
            {"ã±ãJ","‚«‚Í‚ñ"},
            {"¬ˆß","‚Æ‚¤‚¢"},
            {"˜¨—£","‚©‚¢‚è"},
            {"™p–@","‚×‚«‚Ù‚¤"},
            {"‰`ä’","‚¶‚ñ‚º‚ñ"},
            {"˜£Šp","‚©‚ñ‚©‚­"},
            {"çnd","‚µ‚¿‚å‚¤"},
            {"–œÍ","‚Î‚ñ‚±‚­"},
            {"ré˜","‚µ‚ã‚ñ‚Ú‚¤"},
            {"Gèˆ","‚±‚¤‚©‚Â"},
            {"‹Áœô","‚«‚å‚¤‚­"},
            {"¥›W","‚±‚ñ‚µ‚ã"},
            {"æƒé§","‚¯‚ñ‚Æ‚¤"},
            {"æc•Ù","‚Æ‚Â‚×‚ñ"},
        };
        const char* dontknowWord[Qnumber][2] = {
            {"\“ñ’¼","‚±‚æ‚Ý‚Ì‚¿‚ã‚¤‚¾‚ñ"},
            {"‘P’m’¹ˆ«’m’¹","‚¤‚Æ‚¤‚â‚·‚©‚½"},
            {"“¤Â","‚Ý‚é‚¿‚á"},
            {"•½“V","‚Ð‚½‚·‚ç"},
            {"šW","‚Í‚È‚®‚·‚è"},
            {"¬’[","‚Í‚Â‚Í‚Â"},
            {"“y‚é","‚Í‚©‚é"},
            {"—ß‚¿","‚Ì‚è‚²‚¿"},
            {"ŽOŒ·","‚Ë‚±"},
            {"Šy‚¤","‚Ë‚ª‚¤"},
            {"‘¢ŽŸ","‚É‚í‚©"},
            {"”’Šá‚±","‚É‚ç‚ß‚Á‚±"},
            {"Š®›•","‚É‚Á‚±‚è"},
            {"–³‚·‚é","‚È‚Ý‚·‚é"},
            {"’‰Ä•a","‚È‚Â‚â‚¹"},
            {"‰¼Šç","‚È‚Â‚©‚µ"},
            {"”]","‚È‚¸‚«"},
            {"—ŽçÆ","‚È‚·‚Ñ"},
            {"å“®–Ø","‚È‚¬"},
            {"“_","‚È‚©‚Å"},
            {"ŒÕœ","‚È‚©‚Â‚©‚Ý"},
            {"“Ê","‚È‚©‚¾‚©"},
            {"–Ñ‚¢","‚È‚¢"},
            {"ŽU“®","‚Ç‚æ‚Ý"},
            {"ŽüÍ˜T”‚","‚Ç‚¬‚Ü‚¬"},
            {"ŒÓ—£","‚Æ‚ñ‚Ú‚¤"},
            {"ã’¼","‚Æ‚Ü‚è‚Î‚ñ"},
            {"Œû‹V","‚ ‚¢‚³‚Â"},
            {"‹ÈX","‚Æ‚³‚Ü‚©‚­‚³‚Ü"},
            {"ˆ¿Š†","‚Æ‚É‚à‚©‚­‚É‚à"},
            {"—Yæt","‚Æ‚«‚Ì‚±‚¦"},
            {"’·","‚Æ‚±‚µ‚È‚¦"},
            {"“y•—","‚Æ‚±‚ë‚È‚ç‚í‚µ"},
            {"Š¦“yácŸd","‚ ‚©‚ß‚¤‚Ç"},
            {"ž´","‚ ‚©‚ß‚ª‚µ‚í"},
            {"ãœ‹›","‚¢‚©"},
            {"•û","‚¢‚©‚¾"},
            {"Ší—Ê","‚¢‚©‚ß‚µ"},
            {"æÛ","‚¢‚«‚Ü‚«"},
            {"—ÕÕ","‚¢‚­‚³‚®‚é‚Ü"},
            {"‰¼‰‚É","‚¢‚³‚³‚ß‚É"},
            {"×Žq×Žq‚É","‚¢‚³‚ç‚¢‚³‚ç‚É"},
            {"˜ßã\","‚¢‚¿‚á‚¢‚¿‚á"},
            {"”J","‚¢‚Á‚»"},
            {"†ˆ","‚¢‚Á‚Û‚ñ‚¬"},
            {"—¼ã","‚¢‚Â‚í‚è‚²‚Æ"},
            {"‘Å•¯","‚¢‚Å‚½‚¿"},
            {"š¢”","‚¢‚Ô‚«"},
            {"JäÁ","‚¤‚¿‚Ü‚ß"},
            {"‘åŒ`•óŒ•","‚¤‚¿‚í‚³‚Ú‚Ä‚ñ"},
            {"‘ŠŒg‚Ô","‚¤‚Â‚©‚ç‚Ô"},
            {"ˆêØ","‚Â‚â‚Â‚â"},
            {"•‚‹•","‚¨‚¨‚¼‚ç"},
            {"Œ³Œ³‘“¶","‚¨‚¨‚Þ‚½‚©‚ç"},
            {"”çŒÜ’Ü","‚Ä‚Ô‚­‚ë"},
            {"³¿‚è","‚©‚µ‚±‚Ü‚è"},
            {"û‚«","‚©‚µ‚¸‚«"},
            {"—e‚é","‚©‚½‚¿‚Ã‚­‚é"},
            {"™Ê","‚ª‚¯"},
            {"d“ª","‚ª‚³‚Â"},
            {"‹S“•žù","‚«‚Â‚Ë‚Ì‚©‚Ý‚»‚è"},
            {"‰ñ‰ñ•f","‚«‚Â‚Ë‚Ì‚Ú‚½‚ñ"},
            {"èÆèÌáò","‚«‚Â‚Ë‚Ì‚Ü‚³‚©‚è"},
            {"H","‚«‚Ê‚Ê‚¢"},
            {"‘Š","‚«‚Ë‚¤‚½"},
            {"áØ‘R","‚«‚å‚ë‚«‚å‚ë"},
            {"Ÿ ‰","‚«‚ç‚«‚ç"},
            {"˜É‹ü‚¢","‚¬‚±‚¿‚È‚¢"},
            {"„ŠÛ","‚­‚»‚Þ‚µ"},
            {"å”","‚­‚Ñ‚«‚è‚Î‚Á‚½"},
            {"ã‚¦‚é","‚­‚í‚¦‚é"},
            {"‹×‚¢","‚­‚í‚µ‚¢"},
            {"éP","‚¯‚¶‚ß"},
            {"çQ","‚±‚µ‚Ê‚¯"},
            {"’·","‚Æ‚±‚µ‚È‚¦"},
            {"ŽÒ™d","‚±‚Ì‚½‚Ñ"},
            {"ŽŸ™d","‚±‚ñ‚Ç"},
            {"’mŸb","‚µ‚Á‚½‚©"},
            {"ŠŽç","‚µ‚È‚¶‚È"},
            {"—Ñò","‚µ‚Ü"},
            {"–mŽ¡‘ù","‚µ‚ß‚¶‚½‚¯"},
            {"’ð‘R","‚µ‚á‚Á‚«‚è"},
            {"b‰³•¸’š","‚µ‚å‚Ö‚ñ‚É‚Ö‚ñ"},
            {"œŸŒÁ","‚µ‚å‚ñ‚Ú‚è"},
            {"“á•v’m","‚µ‚ç‚¸‚È‚ª‚ç"},
            {"ŽŸX","‚·‚ª‚¢‚·‚ª‚¢"},
            {"Œõ‹V","‚·‚ª‚½"},
            {"úh‚Þ","‚·‚·‚Þ"},
            {"ŽÝ","‚·‚¸‚ß"},
            {"‹›‚é","‚·‚È‚Ç‚é"},
            {"’¬”ò‹r","‚¿‚è‚ñ‚¿‚è‚ñ"},
            {"’‘R","‚¿‚ñ‚Ü‚è"},
            {"œF","‚Â‚¯‚¾‚µ"},
            {"æ§‘i","‚Â‚°‚®‚¿"},
            {"¶•½","‚Æ‚±‚Æ‚í"},
            {"ˆêØ","‚Â‚â‚Â‚â"},
            {"š¢™à","‚Â‚é‚Ì‚­‚Ñ"},
            {"”çŒÜ’Ü","‚Ä‚Ô‚­‚ë"},
            {"‹`Ž¾","‚Å‚ñ‚µ‚Ñ‚å‚¤"},
            {"‰m{","‚Æ‚¨‚©"},
        };
    };

}